...
// ListMilestones list milestones for an organization
func ListMilestones(ctx *context.APIContext) {
	// swagger:operation GET /orgs/{org}/milestones organization orgListMilestones
	// ---
	// summary: List an organization's milestones
	// produces:
	// - application/json
	// parameters:
	// - name: org
...
// CreateMilestone create a milestone for an organization
func CreateMilestone(ctx *context.APIContext) {
	// swagger:operation POST /orgs/{org}/milestones organization orgCreateMilestone
	// ---
	// summary: Create a milestone for an organization
	// consumes:
	// - application/json
	// produces:
	// - application/json
...
// GetMilestone get a milestone for an organization by ID or name
func GetMilestone(ctx *context.APIContext) {
	// swagger:operation GET /orgs/{org}/milestones/{id} organization orgGetMilestone
	// ---
	...
	milestone := getOrgMilestoneByIDOrName(ctx)
	if ctx.Written() {
		return
	}
...
// EditMilestone modify a milestone for an organization
func EditMilestone(ctx *context.APIContext) {
	// swagger:operation PATCH /orgs/{org}/milestones/{id} organization orgEditMilestone
	// ---
...
// DeleteMilestone delete a milestone for an organization
func DeleteMilestone(ctx *context.APIContext) {
	// swagger:operation DELETE /orgs/{org}/milestones/{id} organization orgDeleteMilestone
	// ---
	...
	m := getOrgMilestoneByIDOrName(ctx)
	if ctx.Written() {
		return
	}

	if err := issues_model.DeleteMilestoneByOrgID(ctx, ctx.Org.Organization.ID, m.ID); err != nil {
		ctx.APIErrorInternal(err)
		return
	}
...
// getOrgMilestoneByIDOrName get milestone by ID and if not available by name
func getOrgMilestoneByIDOrName(ctx *context.APIContext) *issues_model.Milestone {
	mile := ctx.PathParam("id")
	mileID, _ := strconv.ParseInt(mile, 0, 64)

	if mileID != 0 {
		milestone, err := issues_model.GetMilestoneByOrgID(ctx, ctx.Org.Organization.ID, mileID)
		if err == nil {
			return milestone
	...
	milestone, err := issues_model.GetMilestoneInOrgByName(ctx, ctx.Org.Organization.ID, mile)
	if err != nil {
